# IntelX — Attack Class Coverage Statement
...
**UI/API wiring note**: the backend has always supported `/api/model/backdoor-probe` and
`/api/model/trigger-reconstruction` independently of the web UI, but the assessment-creation
wizard (`src/client/components/assessment/CreateAssessmentWizard.tsx`) previously only ever
invoked parameter analysis for an uploaded model — a real assessment created through the product's
own UI never exercised known-trigger probing or blind trigger reconstruction, even though both
were fully implemented server-side. The wizard now runs both automatically whenever a model *and*
a dataset are supplied together, using real images pulled from the ingested dataset as probe
images (`/api/dataset/analyze-profile` now returns `probe_sample_image_paths` for exactly this).
This also surfaced and fixed a real, separate bug: `DatasetLoader.load_coco` resolves images
relative to the JSON file's own directory by default, not a sibling `images/` folder, so any
archive following this project's own documented "coco.json + images/" convention silently lost
*all* pixel-based signal (OOD, duplicate hashing, poisoning triggers, and the probe images this
section describes) unless the caller manually passed `images_dir`. `/api/dataset/upload` now
detects and returns `coco_images_dir` when present, and the wizard passes it through automatically
(regression-tested end-to-end via the real HTTP routes in
`test_dataset_archive_upload_detects_sibling_images_dir_for_coco`,
`tests/test_new_capabilities_api.py`).
...
Verified with a test that rotates the audit ledger's signing key _mid-stream_ (one entry signed
...
private key cannot use it to sign a _new_ record after rotation, because the new record's
...
| Attack class         | Status      | Validation method                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| -------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `distribution_shift` | **PARTIAL** | Categorical divergence ratio (terrain/sensor) + illumination delta vs. a declared reference envelope, computed from real per-sample metadata; when an observed sample carries a resolvable `image_path`, real pixel-derived signals are added — blur (Laplacian variance), contrast (intensity std-dev), resolution, and an estimated JPEG-style compression-blockiness score (`backend/drift/image_quality.py`); an embedding-space comparison (real CNN feature vectors, diagonal Frechet distance, `backend/drift/embedding_extractor.py`) is added whenever observed images are supplied AND either (a) a live reference image set is supplied in the same call, or (b) a declared reference baseline (`reference_profile['embedding_centroid']`/`['embedding_variance']`, precomputed once via `EmbeddingExtractor.compute_reference_statistics`) was registered ahead of time — so the embedding signal survives across calls without resupplying raw reference images every evaluation |
